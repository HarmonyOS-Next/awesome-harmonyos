...
| **Alt + 4**                     | **⌘4**             | Run                                       | 显示 或 隐藏 Run。（常用）                                |
| **Alt + 7**                     | **⌘7**             | Structure                                     | 显示 或 隐藏 代码结构树。                            |
| **Ctrl + E**                    | **⌘E**             | Recent Files                                  | 最近的文件（常用，切换文件、面板，强烈推荐）             |
| **Ctrl + P**                    | **⌘P**             | Parameter Info                                | 展示方法的参数信息。（类型提示神器） |
| **Ctrl + Q**                    | 无                    | Quick Documentation                           | 展示组件的 API 说明文档。（查文档神器）                |
...
| **Ctrl + Alt + T**              | **⌥⌘T**         | Surround with…                                | 自动生成具有环绕性质的代码。（生成 `if…else,try…catch` 等代码块） |
...
| **Ctrl + Shift + J**            | **^⇧J**      | Join Lines                                    | 把下一行的代码接续到当前的代码行。（合并行）           |
...
| **快捷键(Win)**       | **快捷键(Mac)**   | **英文说明**            | **中文说明**                                                 |
| --------------------- | ----------------- | ----------------------- | ------------------------------------------------------------ |
| **F8**                | **F8**            | Step  Over              | 跳到当前代码下一行。 （推荐）                                |
| **F7**                | **F7**            | Step  Into              | 跳入到调用的方法内部代码。 （推荐）                          |
| **Alt + F9**          | **⌥F9**        | Run  to Cursor          | 让代码运行到当前光标所在处，非常棒的功能。          |
| **Alt + F8**          | **⌥F8**        | Evaluate  Expression... | 打开一个表达式面板，然后进行进一步的计算。                   |
| **F9**                | **F9**            | Resume  Program         | 结束当前断点的本轮调试（因为有可能代码会被调用多次，所以调用后只会结束当前的这一次）如果有下一个断点会跳到下一个断点中。 |
